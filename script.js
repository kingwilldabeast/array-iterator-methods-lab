...
veryOldInventors = inventors.filter((person) => { return person.year >= 1500 && person.year <= 1599 })
...
inventorNames = inventors.map((person) =>  { return { firstName: person.first, lastName: person.last } })
...
sortedByBirthYear = inventors.sort((personA,personB) => personA.year - personB.year)
...
inventorNamedAda = inventors.find(person => person.first == "Ada")
...
//I worked with Megan Mashburn, Tanner Gilliam, Alfred for the first 9 problems ^^ 
...
totalYearsLived = inventors.map((person) => {return person.passed - person.year}).reduce((a,b) => {return a + b},0)
